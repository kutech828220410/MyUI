...
namespace SLDUI
{
    [System.Drawing.ToolboxBitmap(typeof(E6232), "ECAN.bmp")]
    public partial class E6232 : UserControl
    {
        ...
        private bool FLAG_E6232_Isopen = false;
        private short card_num = 0;
        private short m_nAddr = 0;

        private Form Active_Form;
        private MyThread ThreadStart = new MyThread("E6232Start");
        private MyThread ThreadCompelete = new MyThread("E6232Compelete");
        ...
        private void RefreshUI()
        {
            ...
            exButton_Open.Set_LoadState(FLAG_E6232_Isopen);
        }
        #region Open_E6232
        byte cnt_Open_E6232 = 255;
        void sub_Open_E6232()
        {
            if (cnt_Open_E6232 == 1) cnt_Open_E6232_00_檢查控制器是否開啟(ref cnt_Open_E6232);
            if (cnt_Open_E6232 == 2) cnt_Open_E6232_00_檢查連接狀態(ref cnt_Open_E6232);
            if (cnt_Open_E6232 == 3) cnt_Open_E6232_00_開啟板卡(ref cnt_Open_E6232);
            if (cnt_Open_E6232 == 4) cnt_Open_E6232 = 240;

            if (cnt_Open_E6232 == 240) cnt_Open_E6232_240_開啟成功(ref cnt_Open_E6232);
            if (cnt_Open_E6232 == 241) cnt_Open_E6232 = 255;

            if (cnt_Open_E6232 == 250) cnt_Open_E6232_250_開啟失敗(ref cnt_Open_E6232);
            if (cnt_Open_E6232 == 251) cnt_Open_E6232 = 255;

        }
        void cnt_Open_E6232_00_檢查控制器是否開啟(ref byte cnt)
        {
            FLAG_E6232_Isopen = false;
        ...
        void cnt_Open_E6232_00_檢查連接狀態(ref byte cnt)
        {
        ...
        void cnt_Open_E6232_00_開啟板卡(ref byte cnt)
        {
        ...
        void cnt_Open_E6232_240_開啟成功(ref byte cnt)
        {
            string temp0 = "";
            string device_type0 = "";
            for (int k = 0; k < chanel_num; k++)
            ...
            SaveProperties();    
            FLAG_E6232_Isopen = true;
            cnt++;
        }
        void cnt_Open_E6232_250_開啟失敗(ref byte cnt)
        {
            SLD_Basic.ErrorCode(SLDMclass.SLDM_Close(card_num), true);
            cnt++;
        }
        void cnt_Open_E6232_(ref byte cnt)
        {
            cnt++;
        }
        #endregion
        #region Close_E6232
        byte cnt_Close_E6232 = 255;
        void sub_Close_E6232()
        {
            if (cnt_Close_E6232 == 1)
            {
                SLD_Basic.ErrorCode(SLDMclass.SLDM_Close(card_num), true);
                FLAG_E6232_Isopen = false;
                for (int k = 0; k < chanel_num; k++)
        ...
        void sub_Get_Input()
        {
            if (FLAG_E6232_Isopen)
            {
                ...
                for (int k = 0; k < chanel_num; k++)
                {
                    for (int i = 0; i < 8; i++)
                    {
                        SLDMclass.SLDM_GetInBit(card_num, m_nAddr, (short)((k * 8) + i), ref flag);
            ...
            if (FLAG_E6232_Isopen)
            {
                ...
                for (int k = 0; k < chanel_num; k++)
                {
                    for (int i = 0; i < 8; i++)
                    {
                        ...
                        SLDMclass.SLDM_SetOutBit(card_num, m_nAddr, (short)((k * 8) + i), flag);
        ...
        void sub_WriteToPLC()
        {
            if (FLAG_E6232_Isopen && lowerMachine != null)
            {
                bool flag = false;
                string device = "";
        ...
        private void exButton_Open_btnClick(object sender, EventArgs e)
        {
            ...
            {
                if (cnt_Open_E6232 == 255) cnt_Open_E6232 = 1;
            }
            else
            {
                if (cnt_Close_E6232 == 255) cnt_Close_E6232 = 1;
            } 
        }
        [Serializable]
