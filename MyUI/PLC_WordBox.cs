...
namespace MyUI
{
    ...
    public partial class PLC_WordBox : TextBox
    {
        ...
        public override System.Windows.Forms.Layout.LayoutEngine LayoutEngine
        {
            get
            {
                if (!flag_init)
                {
                    textBox1.UseSystemPasswordChar = _密碼欄位;
                    GetInitStr();
                }
        ...
        public PLC_WordBox(IContainer container)
        {
            InitializeComponent();
        }
        ...
        {
            SetString(textBox1.Text);
        }

        private void textBox1_MouseDown(object sender, MouseEventArgs e)
        {
            if (顯示螢幕小鍵盤 && !MyUI.字母鍵盤.視窗已建立)
            {
                MyUI.字母鍵盤.音效 = 音效;
                MyUI.字母鍵盤.密碼字元 = 密碼欄位;
                MyUI.字母鍵盤.字節數 = int_字節數DATA;
                MyUI.字母鍵盤.InitText = this.Text;
                MyUI.字母鍵盤 form = MyUI.字母鍵盤.GetForm();
